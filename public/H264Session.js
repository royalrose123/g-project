    ...
    remuxRTPData (rtspInterleaved, rtpHeader, rtpPayload) {
      ...
      switch (nalType) {
        case 6: // SEI
          ...
          if (PAYLOAD[1] === 0x55 && PAYLOAD[2] === 0x4c) {
            // console.log(rtpTimeStamp, rtpHeader[4], rtpHeader[5], rtpHeader[6] , rtpHeader[7])
            // console.log('SEI timeStamp: ', rtpTimeStamp)
          }
        ...
        case 28: // FU
          ...
          let endBit = (PAYLOAD[1] & 0x40) === 0x40

          let fuType = PAYLOAD[1] & 0x1f
      ...
      if (RtpHeader.M) {
        ...
        if (rtpTimeStamp < lastTimeStamp && lastTimeStamp - rtpTimeStamp > RESETTIME / 2) {
          // 判断lastTimeStamp远大于rtpTimeStamp，防止后一帧比前一帧先到的情况
        ...
        if (SEIInfo.timestamp === RtpHeader.timeStamp) {
          ...
          // this.handleDecodedData(SEIInfo);
          // return;

          // console.log(rtpTimeStamp - lastTime);
          if (!(rtpTimeStamp - lastTime)) {
            console.log('上个RTP包时间和本次相等')
            console.log(
              'lastTime: ',
              lastTime,
              ' rtpTimeStamp: ',
...
function parseRect (data) {
  ...
  while (dataLength > 0) {
    ...
    let y0 = ((data[6] & 0x7f) << 8) + data[7]

    let width = ((data[8] & 0x7f) << 8) + data[9] - x0

    let height = ((data[10] & 0x7f) << 8) + data[11] - y0
