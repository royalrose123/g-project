...
//console.log(initSegmentData)
                ...
                videoMS.init();
                videoMS.onCanplayCallback(()=>{ivsDrawer.cover(videoElement); onCanplayCallback && onCanplayCallback()});
...
//console.log(mediaInfo);
    ...
    function getIVS(timestamp) {
        //console.log(parseInt(timestamp - lastTime));
        //lastTime = timestamp;
        //console.log(SEIinfo)
        //SEIinfo.length() ? console.log(SEIinfo.top()) : null;
        //console.log(SEIinfo.length())
        let preNode = null;
        let nextNode = null;
        //preNode = SEIinfo.top();
        ...
        while((preNode !== undefined) && (preNode !== null)) {
            if(preNode[0] > timestamp) {
                ...
                if(nextNode === undefined || nextNode === null) {
                    //if(preNode[1] !== null) {
                    //console.log('最后一个node', timestamp, preNode, SEIinfo.length());
                    //}
    ...
    function drawIVS(timestamp) {
        ...
        if(data === undefined || data === null) {
            //清空画布
            //console.log('无ivs信息', timestamp, SEIinfo.length());
            // if(SEIinfo.length()) {
            //     console.log('找不到IVS信息', timestamp)
            //     SEIinfo.map((v, k)=>{
            //         console.log(v);
            //     });
            // }
        ...
        // if(SEIinfo.top() !== undefined) {
        //     let data = SEIinfo.shift()
        //     ivsDrawer.draw(data[1])
        //     console.log(timestamp,data[0] );
        //     //console.log(data)
        //     //console.log('第二种: ', SEIinfo.top()[0]);
        //
        // }else {
        //     ivsDrawer.clearCanvas();
        // }
